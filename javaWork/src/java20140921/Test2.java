		...
		String table[][] 
			...
			for(int x=0;x<2;x++){ //x를 2로 저장하여 2열 반복 
				...
				do{ //jumsu에 입력받을 수가 '999'가 아닐 때 반복 실행
					...
					jumsu = Integer.parseInt(inbr.readLine()); //임의의수 입력
			
					if(jumsu < 0) //입력받은 jumsu가 0보다 작을 때 재입력 메시지 출력
						System.out.println("*"+jumsu+" ->  성적을 올바르게 입력하세요!");
					else if(jumsu>100 && jumsu<999)//입력받은 jumsu가 100보다 크고 999보다 작을 때 재입력 메시지 출력
						System.out.println("*"+jumsu+" ->  성적을 올바르게 입력하세요!");
					else if(jumsu>999)//입력받은 jumsu가 999보다 클 때 재입력 메시지 출력
						System.out.println("*"+jumsu+" ->  성적을 올바르게 입력하세요!");
			
		
					else if(jumsu<=100 && jumsu>=Integer.parseInt(table[0][0])) //범위에 따른 성적 출력
